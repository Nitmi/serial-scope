            ...
            egui::Frame::default()
                ...
                .show(ui, |ui| {
                    ...
                    ui.horizontal_wrapped(|ui| {
                        ...
                        if baud_response.lost_focus() && ui.input(|i| i.key_pressed(egui::Key::Enter)) {
                        ...
                        ui.separator();
                        let open_btn = ui.add_enabled(!app.is_connected, egui::Button::new("打开串口"));
                        if open_btn.clicked() {
                            app.apply_baud_rate_input();
                        ...
                        let close_btn = ui.add_enabled(app.is_connected, egui::Button::new("关闭串口"));
                    ...
                    ui.horizontal_wrapped(|ui| {
                        ...
                        ComboBox::from_id_salt("parser_mode")
                            ...
                            .show_ui(ui, |ui| {
                                for mode in ParserMode::ALL {
                                    if ui
                                        .selectable_value(&mut app.config.parser.mode, mode, mode.label())
                                        .changed()
                                    {
                                        app.persist_config();
                                    }
                        ...
                        if ui
                            .add(egui::TextEdit::singleline(&mut delimiter_text).desired_width(40.0))
                            .changed()
                        {
                        ...
                        if ui
                            .add(
                                egui::TextEdit::singleline(&mut app.config.parser.csv_channel_names)
                                    .desired_width(220.0),
                            )
                            .changed()
                        {
                    ...
                    ui.horizontal_wrapped(|ui| {
                        ...
                        ui.add(egui::TextEdit::singleline(&mut app.export_base_name).desired_width(180.0));
                    ...
                    ui.horizontal_wrapped(|ui| {
                        ...
                        if let Some(error) = &app.last_error {
                            ui.separator();
                            ui.label(RichText::new(error).color(egui::Color32::from_rgb(255, 128, 128)));
                        }
                    });
                });
        });
